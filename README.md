...
3. **MAIN** — exatamente 39 cartas dos tipos `UNIT` ou `SPELL`.
4. **RUNE** — exatamente 12 cartas do tipo `RUNE`.
5. **BATTLEFIELD** — exatamente 3 cartas do tipo `BATTLEFIELD`.
6. **SIDEBOARD** — até 8 cartas adicionais dos tipos `UNIT` ou `SPELL`.
7. **SIDEBOARD_BATTLEFIELD** — até 2 cartas adicionais do tipo `BATTLEFIELD`.
8. **Limite de cópias** — no máximo 3 cópias da mesma carta entre Main e Sideboard, considerando as exceções aplicáveis a Runas, Lendas e Campeões.
9. **Cartas banidas** — cartas marcadas como banidas impedem a validação do deck.
10. **Compatibilidade de facção** — verificação de compatibilidade das cartas com as facções definidas pela Lenda e pelo Campeão.
